...
public class DaysSincePriceThreshold {
    public static void main(String[] args) throws Exception {
        ...
        closePrices.keyBy(value -> value.f0).flatMap(new NumberOfDaysSinceLastThresholdBreachFn()).print();
        env.execute();
    }

    public static class NumberOfDaysSinceLastThresholdBreachFn extends RichFlatMapFunction<Tuple3<String, String, Double>, Tuple2<String, Integer>>{

        private transient ListState<Double> pricesListState;
        @Override
        public void flatMap(Tuple3<String, String, Double> input, Collector<Tuple2<String, Integer>> collector) throws Exception {
            if(input.f2 > 180){
        ...
        public void open(Configuration config){
            ListStateDescriptor<Double> descriptor = new ListStateDescriptor<Double>("daysSinceThreshold", Double.class);
